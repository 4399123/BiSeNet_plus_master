...
from .fbnetv3_g import FBNetV3_G
        ...
        for pool_size in pool_sizes:
            self.append(
                nn.Sequential(
                    ...
                    nn.Conv2d(self.in_channels, self.out_channels, kernel_size=(1,1),bias=False),
                )
            )
...
class PSPHEAD(nn.Module):
    def __init__(self, in_channels, mid_channels,outchannels,pool_sizes=[1, 2, 3, 6]):
        ...
        self.final = nn.Sequential(
            nn.BatchNorm2d(self.in_channels + len(self.pool_sizes) * self.out_channels),
            nn.ReLU(inplace=True),
        ...
        self.shortcut = nn.Sequential(
                                    ...
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(in_channels, outchannels, kernel_size=1, bias=False),
                                    )
    ...
    def forward(self, x):
        ...
        out.append(x)
        out = torch.cat(out, 1)
        out = self.final(out)+self.shortcut(x)
        return out
...
if __name__ == "__main__":
    net = BiSeNetV1_FBNetV3_G_PPM(19)
    net.eval()
