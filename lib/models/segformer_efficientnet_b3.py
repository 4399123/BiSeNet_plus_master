...
class Segformer_EfficientNet_B3(nn.Module):
    def __init__(self,
        dims = (32, 48, 136, 384),
        decoder_dim = 256,
        n_classes=9,
        ...
        super(Segformer_EfficientNet_B3,self).__init__()
        dims=tuple(dims)

        self.out_indices = [1,2,3,4]
        self.selected_feature_extractor = timm.create_model('tf_efficientnet_b3.ns_jft_in1k',
                                                            features_only=True,
        ...
        if self.aux_mode == 'train':
            self.conv_out16 = BiSeNetOutput(48, decoder_dim, n_classes, up_factor=8)
            self.conv_out32 = BiSeNetOutput(136, decoder_dim, n_classes, up_factor=16)
    ...
    def forward(self, x):
        with autocast(enabled=self.use_fp16):
            ...
            if self.aux_mode == 'train':
                feat_out8 = self.conv_out16( layer_outputs[1])
                feat_out16 = self.conv_out32(layer_outputs[2])
...
if __name__ == "__main__":
    net = Segformer_EfficientNet_B3(n_classes=9)
    net.eval()
